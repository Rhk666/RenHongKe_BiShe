...
{
//	 esp8266_buf[0] = '\0'; // 简单把第一个字节置0，防止 strstr 读到旧数据
////    memset(esp8266_buf, 0, sizeof(esp8266_buf)); // 清数据
//    esp8266_cnt = 0;                               // 清当前计数
//    esp8266_cntPre = 0;                            // 【关键！！！加上这一行】清前一次计数
    esp8266_cnt = 0;
    esp8266_cntPre = 0;
    // 只清前100字节，保留后续接收能力，不彻底清光导致断联
...
/**********************************************************
* 函  数 名: ESP8266_GetIPD_RTOS
* 功能说明: 专门用于FreeRTOS任务的接收函数，使用 vTaskDelay
**********************************************************/
unsigned char *ESP8266_GetIPD_RTOS(unsigned short timeOut)
{
    char *ptrIPD = NULL;
    ...
    GyroRawData_t send_data;  
		...
		else if (Rxstate == 2)
		{
			if (Temp_Data == 0x55)
			{
				...
				Rxstate = 0;
				for (int i=0; i<8; i++) {
          send_data.data[i] = Rx_Data[i];
         }
         xQueueSendFromISR(
           xGyroQueue,        // 队列句柄
           &send_data,        // 要发送的数据
           &xHigherPriorityTaskWoken  // 标记是否需要调度
            );				
