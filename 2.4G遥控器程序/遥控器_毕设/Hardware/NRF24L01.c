...
void TIM2_Init(){
	...
	TIMBASEINIT.TIM_Period=100 - 1;
	TIMBASEINIT.TIM_Prescaler=720 - 1;
	...
	NVIC_InitTypeDef NVICSTRUCA;
	
	NVICSTRUCA.NVIC_IRQChannel=TIM2_IRQn;
	NVICSTRUCA.NVIC_IRQChannelCmd=ENABLE;
	NVICSTRUCA.NVIC_IRQChannelPreemptionPriority=2;
	NVICSTRUCA.NVIC_IRQChannelSubPriority=1;
	
	
	NVIC_Init(&NVICSTRUCA);
...
void TIM3_Init(){
	...
	NVICSTRUCA.NVIC_IRQChannel=TIM3_IRQn;
	NVICSTRUCA.NVIC_IRQChannelCmd=ENABLE;
	NVICSTRUCA.NVIC_IRQChannelPreemptionPriority=2;
	NVICSTRUCA.NVIC_IRQChannelSubPriority=0;
...
#define NRF24L01_RX_PACKET_WIDTH		9							//接收通道0数据包宽度，范围：1~32字节
